...
// Подключаем анализаторы изображений
let advancedImageAnalyzer, smartVisionAnalyzer, smartObjectDetector;

try {
  advancedImageAnalyzer = require('./advanced-image-analyzer.js');
  smartVisionAnalyzer = require('./smart-vision-analyzer.js');
  smartObjectDetector = require('./smart-object-detector.js');
} catch (error) {
  console.log('⚠️ [VISUAL-SEMANTIC] Некоторые анализаторы изображений недоступны:', error.message);
}
