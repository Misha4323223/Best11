...
const emotionalAnalyzer = {
  ...
  getResponseTemplates(tone, category) {
    // Для обычного общения используем более естественные шаблоны
    if (category === 'conversation') {
      const naturalTemplates = {
        joy: [
          { prefix: 'Отлично!', suffix: '😊' },
          { prefix: 'Замечательно!', suffix: '✨' },
          { prefix: 'Здорово!', suffix: '🌟' }
        ],

        anger: [
          { prefix: 'Понимаю.', suffix: 'Надеюсь, всё наладится.' },
          { prefix: 'Сочувствую.', suffix: 'Постараюсь помочь.' },
          { prefix: 'Вижу, что расстроены.', suffix: '' }
        ],

        sadness: [
          { prefix: 'Понимаю, что сейчас непросто.', suffix: '🌈' },
          { prefix: 'Не унывайте!', suffix: 'Всё будет хорошо! 💝' },
          { prefix: 'Поддерживаю вас!', suffix: '🤗' }
        ],

        surprise: [
          { prefix: 'Интересно!', suffix: '🔍' },
          { prefix: 'Необычно!', suffix: '🧐' },
          { prefix: 'Любопытно!', suffix: '✨' }
        ],

        polite: [
          { prefix: 'Конечно!', suffix: '🤝' },
          { prefix: 'С удовольствием!', suffix: '😊' },
          { prefix: 'Разумеется!', suffix: '🙏' }
        ],

        neutral_question: [
          { prefix: 'Хороший вопрос!', suffix: '💭' },
          { prefix: 'Понятно!', suffix: '🎯' },
          { prefix: 'Интересно!', suffix: '📋' }
        ],
        ...
        neutral: [
          { prefix: '', suffix: '' },
          { prefix: 'Понятно!', suffix: '' },
          { prefix: 'Хорошо!', suffix: '' }
        ]
      };

      return naturalTemplates[tone] || naturalTemplates.neutral;
    }

    // Для других категорий (генерация изображений, анализ и т.д.) используем полные шаблоны
    const templates = {
      joy: [
...
async function executeConversationPlan(userQuery, options) {
  ...
  try {
    ...
    // НОВАЯ ЛОГИКА: Проверяем тип сообщения для более естественных ответов
    const query = userQuery.toLowerCase().trim();
    
    // Простые приветствия - короткий естественный ответ
    if (query.includes('привет') || query.includes('hello') || query.includes('hi')) {
      const greetings = [
        'Привет! Как дела?',
        'Привет! Рад снова общаться!',
      ...
      reasons.push('Использовал естественное приветствие без навязчивых предложений');
      const finalReason = reasons.join(' → ');

      return {
        success: true,
        response: adaptedResponse,
        provider: 'IntelligentConversationNatural',
        category: 'conversation',
    ...
    // Вопросы "как дела" - персональный ответ
    if (query.includes('как дела') || query.includes('как ты') || query.includes('how are you')) {
      const responses = [
        'У меня всё отлично! А у тебя как дела?',
        'Всё хорошо, спасибо! Как твои дела?',
        'Отлично! Готов помочь. Как у тебя дела?',
        'Прекрасно! А что у тебя нового?'
      ];
      ...
      reasons.push('Использовал персональный ответ на вопрос о делах');
      const finalReason = reasons.join(' → ');
    ...
    let conversationPrompt = `Ты дружелюбный AI-помощник. Отвечай естественно, как человек в дружеской беседе. НЕ перечисляй свои возможности, если пользователь не спрашивает о них напрямую. `;
...
ВАЖНО: Отвечай кратко и естественно. НЕ предлагай список возможностей, если пользователь не спрашивает "что ты умеешь?" или подобное.

Пользователь: "${userQuery}"

Ответь естественно, как в обычной беседе.`;
