...
class ExternalKnowledgeIntegrator {
  ...
  async enrichWithExternalKnowledge(query, context = {}) {
    ...
    try {
      ...
      // 3. Поиск в краудсорсинговых аннотациях
      ...
      // 4. Поиск связанных концепций в реальновременном графе
      ...
      const result = {
        ...
        enrichedContext: this.createEnrichedContext(
          wikipediaResults, scientificResults, crowdsourcedResults, relatedConcepts
        ),
        ...
        // Метрики
        enrichmentMetrics: {
          totalSources: wikipediaResults.length + scientificResults.length + crowdsourcedResults.length,
