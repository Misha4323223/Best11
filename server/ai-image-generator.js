...
async function getAIEnhancedPrompt(prompt, style) {
  ...
  // 🧠 НОВИНКА: Пытаемся использовать интеллектуальный улучшатель
  try {
    const intelligentEnhancer = require('./intelligent-pollinations-enhancer.js');
    console.log(`🧠 [AI-PROMPT] Использую интеллектуальный улучшатель`);
    
    const enhancementResult = await intelligentEnhancer.enhanceImageGeneration(prompt, {
      style,
      prioritizeSpeed: true, // для быстрого ответа
      allowMultipleVariants: false // только одно изображение
    });
    
    if (enhancementResult.success) {
      console.log(`✨ [AI-PROMPT] Интеллектуальное улучшение успешно`);
      return enhancementResult.enhancedPrompt;
    } else {
      console.log(`⚠️ [AI-PROMPT] Интеллектуальное улучшение не удалось, переходим к базовому AI`);
    }
  } catch (enhancerError) {
    console.log(`⚠️ [AI-PROMPT] Интеллектуальный улучшатель недоступен: ${enhancerError.message}`);
  }
