...
async function getAIResponseWithSearch(userQuery, options = {}) {
  try {
    ...
    // ===== СЕМАНТИЧЕСКАЯ ИНТЕГРАЦИЯ (ГЛАВНЫЙ МОЗГ) =====
    // Семантическая память теперь главный анализатор системы
    SmartLogger.route(`🧠 Запуск семантического анализа как главного мозга`);
    try {
      const semanticAnalysis = await semanticIntegration.analyzeWithSemantics(userQuery, options);

      if (semanticAnalysis.shouldUseSemantic) {
        SmartLogger.route(`✅ Семантика берет управление (${semanticAnalysis.reason})`);
        
        // Создаем семантически обогащенный ответ
        const semanticResponse = await semanticIntegration.createSemanticResponse(
          semanticAnalysis.semanticResult, userQuery, options
        );
        
        if (semanticResponse.success) {
          SmartLogger.route(`🎯 Семантический ответ создан успешно`);
          return semanticResponse;
        } else {
          SmartLogger.route(`⚠️ Семантический ответ не удался, переход к процессору`);
        }
      } else {
        SmartLogger.route(`⚠️ Семантика передает управление процессору (${semanticAnalysis.reason})`);
      }

      // ===== ИНТЕЛЛЕКТУАЛЬНЫЙ ПРОЦЕССОР (ИСПОЛНИТЕЛЬ) =====
      // Процессор теперь работает как исполнитель семантических решений
      SmartLogger.route(`🔧 Запуск интеллектуального процессора как исполнителя`);
      
      // Передаем семантические данные процессору для обогащения
      const enrichedOptions = {
        ...options,
        semanticData: semanticAnalysis.semanticResult
      };
      
      const intelligentResult = await intelligentProcessor.analyzeAndExecute(userQuery, enrichedOptions);

      if (intelligentResult.success) {
        SmartLogger.route(`✅ Интеллектуальный процессор успешно обработал запрос`);
        
        // Обогащаем ответ процессора семантическими данными
        if (semanticAnalysis.semanticResult) {
          const enrichedResult = semanticIntegration.enrichStandardResponse(
            intelligentResult, semanticAnalysis.semanticResult
          );
          SmartLogger.route(`🔗 Ответ обогащен семантическими данными`);
      ...
      SmartLogger.route(`⚠️ Ошибка семантической интеграции: ${semanticError.message}, переход к стандартной логике`);
      // Продолжаем выполнение стандартной логики
    }
    // ===== КОНЕЦ СЕМАНТИЧЕСКОЙ ИНТЕГРАЦИИ =====

    // ===== FALLBACK АНАЛИЗ САЙТОВ =====
    // Если интеллектуальный процессор не справился, проверяем анализ сайтов вручную
    try {
      ...
      if (detection.isWebsiteAnalysis && detection.confidence >= 60) {
        SmartLogger.route(`🔍 Fallback: обнаружен запрос анализа сайта (уверенность: ${detection.confidence}%)`);
        
        const analysisResult = await websiteAnalyzer.analyzeWebsite(userQuery, options);
        
        if (analysisResult.success) {
          SmartLogger.route(`✅ Fallback анализ сайта успешно выполнен`);
          return {
            success: true,
            response: analysisResult.response,
            provider: 'FallbackWebsiteAnalyzer',
            category: 'website_analysis',
            url: analysisResult.url,
            websiteData: analysisResult.businessAnalysis
          };
        } else {
          SmartLogger.route(`❌ Fallback анализ сайта не удался: ${analysisResult.error}`);
        }
      }
    } catch (fallbackError) {
      SmartLogger.route(`⚠️ Ошибка fallback анализа сайтов: ${fallbackError.message}`);
    }
    // ===== КОНЕЦ FALLBACK АНАЛИЗА САЙТОВ =====
